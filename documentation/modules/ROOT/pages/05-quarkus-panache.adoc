...
* [x] Explain what Panache is and how it relates to the Hiberate Object Relational Mapper (ORM)
* [x] Build a Hibernate application using Panache

[#quarkusp-demo-overview]
== Demo Overview 

You'll learn how easy and productive Quarkus is with Hibernate and Panache.  For this, we'll develope an application called "Fruits".  It's a simple CRUD REST API that handles information about fruits.

We'll use https://mariadb.org/[MariaDB] as our backing database.

[#quarkusp-fruitapp-dev]
=== Build and Deploy

First, we need a running instance of our database:
...
----
kubectl apply -n {tutorial-namespace} -f pass:[$TUTORIAL_HOME]/kubernetes-app/mariadb.yaml
----
...
[source,bash,subs="+macros,+attributes"]
----
oc apply -n {tutorial-namespace} -f pass:[$TUTORIAL_HOME]/kubernetes-app/mariadb.yaml
----
...
Any of the credentials below can be used to connect to the database:
...
Open a new terminal window and run the following command (blocking):
...
# <3>
%fromtools.quarkus.datasource.url = jdbc:mariadb://mariadb:3306/demodb
%test.quarkus.datasource.url=jdbc:mariadb://127.0.0.1:3306/demodb
...
<2> URL used when app is running in development mode via tutorial-tools

NOTE: In tutorial-tools, we use the mariadb DNS name using the profile `fromtools`. Start Quarkus dev with this command `./mvnw compile quarkus:dev -Dquarkus.profile=fromtools`.
...
Making the `Fruit` entity extend `Panache Entity` will make it both Entity and DAO which will resolve the error in the `FruitResource.java`.
...
[source,bash,subs="+macros,+attributes"]
----
./mvnw quarkus:add-extension -Dextension="openapi"
----
...
To validate simple output messages RestAssured assertions mechanism is enough but in our experience, when one starts dealing with more complex assertions then things start to become a bit more complex. 
...
Open the class `$PROJECT_HOME/src/test/java/com/example/FruitResourceTest.java` and replace *all* the content with the following:
